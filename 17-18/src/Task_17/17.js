...
fs.readFile(
	...
	async (err, data) => {
		...
		for (let i = 0; i < polygons.length; i++) {
			vertexCount[polygons[i][0]] += 1 //polygons[i][0] это номер вершины которая есть в полигоне
			vertexCount[polygons[i][1]] += 1
			vertexCount[polygons[i][2]] += 1
		}
		try {
